...
class findattribute(nn.Module):
    def __init__(self, tokenizer, drop_rate=0, gpu=True):
        ...
        # self.PAD_IDX = 50258
        self.EOS_IDX = tokenizer.encode(tokenizer.eos_token)[0]
        self.START_IDX = tokenizer.encode(tokenizer.bos_token)[0]
        self.PAD_IDX = tokenizer.encode(tokenizer.pad_token)[0]
    ...
    def unbatch_discriminator(self, token_idx):
        """
    ...
    def discriminator(self, token_idx):
        ...
        #         padding_token = padding_token.cuda()
        #     token_idx=torch.cat([token_idx,padding_token], 1) # (batch, seq_len+padding) = (batch, 5)
        # token_idx = torch.tensor([[x if x != 3 else self.PAD_IDX for x in token_idx[y]] for y in range(len(token_idx))]).cuda()
        token_idx = torch.tensor(token_idx).cuda()
        word_emb = self.word_emb(token_idx)  # (batch, seq_len, word_dim)
        word_2d = word_emb.unsqueeze(1)  # (batch, 1, seq_len, word_dim)
    ...
    def unbatch_att_prob(self, token_idx, sentiment):
        """
        token_idx: (batch, seq_len)
        """
        ...
        for i in range(len(token_list)):
            ...
            del_prob = F.softmax(self.discriminator(del_tensor), 1).squeeze(0)[
                sentiment].cpu().detach().numpy().item()
    ...
    def att_prob(self, token_idx, sentiment):
        ...
        min_prob = 1
        del_idx_list = []
        for batch in range(len(token_list)):
            # for i in range(len(token_list)):
            for i in range(len(token_list[batch])):
                del_list = token_list[batch][:i] + token_list[batch][i + 1:]
                del_tensor = torch.from_numpy(np.asarray(del_list)).unsqueeze(0).cuda()
                del_prob = F.softmax(self.discriminator(del_tensor), 1).squeeze(0)[
                    sentiment[batch]].cpu().detach().numpy()#.item()

                if del_prob <= min_prob:
                    max_ind = i
                    min_prob = del_prob

            final_list = token_list[batch][:max_ind] + token_list[batch][max_ind + 1:]
            del_idx_list.append(final_list)
            # del_idx = torch.from_numpy(np.asarray(final_list)).unsqueeze(0).cuda()
        del_idx = torch.from_numpy(np.array(del_idx_list)).cuda()
        return del_idx
