...
class Loader:
    ...
    def readTxt(self, path, data_path):
        df = pd.read_csv('/'.join([path, data_path]), encoding='utf-8')
        std_data = df['std'][:30000]
        dia_data = df['dia'][:30000]

        self.pairs = [[self._normalize(std), self._normalize(dia)] for std, dia in zip(std_data, dia_data)]
        self._filterPairs()
